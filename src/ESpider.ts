...
import {everyHasKeys, getRandomItemForArray, sleep} from "./utils/methods";
...
import {RequestDupeFilter} from "./lib/RequestFilter";
...
export class ESpider implements SpiderMiddleware {
  ...
  public fingerprint: RequestDupeFilter
  ...
  public setOptions(opt: Partial<ESpiderOptions> = {}): this {
    ...
    for (const optKey in opt) {
      ...
      if (everyHasKeys(this, opt, [optKey])) this[optKey] = opt[optKey]
    }
    ...
    if (opt.requestFilterOptions) this.fingerprint.setOptions(opt)
    return this
  }
    ...
    await this.requestQueue.add(async () => {
      ...
      if (!req) {
        this.requestQueueModel.findOne({
          ...
          if (dbRes) {
            dbResRef = dbRes
            callMiddleware('onRequestTask', this, null, async (cb) => {
